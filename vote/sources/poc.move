module itc_poc_v3::poc {
    ...
    const EThresholdNotReached: u64 = 13;
    const ERewardAlreadyClaimed: u64 = 14;
    const ESubmissionNotApproved: u64 = 15;
    ...
    public struct TaskSubmission has key {
        ...
        vote_count: u64,
        voters: Table<address, bool> // tracks who voted and if they claimed reward (true = claimed)
    }
    ...
    public fun vote_submission(
        ...
        assert!(now <= task.deadline || task.is_competition, EVotingFinished); // In competition, voting is after deadline. In normal curation, it's during task.
        if (task.is_competition) {
        ...
        // New logic: 1 vote per submission
        assert!(!table::contains(&submission.voters, sender), EAlreadyVoted);

        table::add(&mut submission.voters, sender, false); // false = reward not claimed
        submission.vote_count = submission.vote_count + 1;

        // Still update top submission for competition mode
        if (task.is_competition && submission.vote_count > task.max_votes) {
    ...
    /// Community can finalize a submission if it reaches 5 votes
    public fun community_finalize(
        submission: &mut TaskSubmission,
        profile: &mut StudentProfile,
        task: &Task,
        config: &mut GlobalConfig,
        ...
        assert!(submission.vote_count >= 5, EThresholdNotReached);
        assert!(submission.status == STATUS_PENDING, ESubmissionAlreadyApproved);
        assert!(submission.task_id == object::id(task), ENotAuthorized);
        assert!(submission.student_id == object::id(profile), ENotAuthorized);

        submission.status = STATUS_APPROVED;
        submission.comment = utf8(b"Approved by Community (5+ votes)");
        ...
        let contribution = Contribution {
            ...
            points: task.points,
            timestamp: sui::clock::timestamp_ms(clock),
            verified_by: utf8(b"Community Vote")
        };
    ...
    /// Claim reward for being a curator (+5 XP)
    public fun claim_curator_reward(
        curator_profile: &mut StudentProfile,
        submission: &mut TaskSubmission,
        ctx: &mut TxContext
    ) {
        let sender = tx_context::sender(ctx);
        assert!(curator_profile.owner == sender, ENotAuthorized);
        assert!(submission.status == STATUS_APPROVED, ESubmissionNotApproved);
        assert!(table::contains(&submission.voters, sender), ENotAuthorized);
        
        let claimed = table::borrow_mut(&mut submission.voters, sender);
        assert!(!*claimed, ERewardAlreadyClaimed);
        
        *claimed = true; // Mark as claimed

        // Reward the curator with 5 points/XP
        let reward_points = 5;
        curator_profile.total_points = curator_profile.total_points + reward_points;
        // Also boost reputation
        curator_profile.reputation = if (curator_profile.reputation + 1 > MAX_REPUTATION) MAX_REPUTATION else curator_profile.reputation + 1;

        event::emit(ContributionAdded {
            profile_id: object::id(curator_profile),
            title: utf8(b"Curator Reward"),
